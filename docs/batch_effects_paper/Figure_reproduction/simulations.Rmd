...
```{r}
R <- 1000
eff_sz.settings <- c("positive"=1/2, "null"=0, "negative"=-1/2)
...
sim_results <- do.call(rbind, lapply(names(eff_sz.settings), function(j) {
  ...
  sim.cate.settings <- lapply(1:length(betas), function(i) {
    list(alpha=2, unbalancedness=betas[i], eff_sz=eff_sz)
  })
...
ggplot(data=merged.longdat, aes(x=Overlap, y=Mean)) +
    #geom_ribbon(data=merged.longdat, aes(ymin=lpt, ymax=upt, fill=Technique), alpha=0.4) +
    ...
    geom_hline(data=hline.dat, aes(yintercept=True.Effect), color="black", linetype="dashed") +
    geom_hline(aes(yintercept=0), color="black", linetype="dotted") +
    scale_x_continuous(name="Covariate Overlap", trans="log2", 
                       breaks=c(0.01, 0.1, 1)) +
    scale_y_continuous(name="|CATE|", limits=c(0, 3.1), expand=c(0, 0)) +
    theme_bw() +
