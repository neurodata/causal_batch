...
dset.ord.site <- readRDS("../data/corr_fmri_metadata.rds") %>%
  filter(!(Dataset %in% c("NKI24tr1400", "NKI24tr645")))

results <- (rawdat = readRDS("../data/pairwise.rds") ) %>%
  filter(!(Dataset.Trt %in% c("NKI24tr645", "NKI24tr1400")), !(Dataset.Ctrl %in% c("NKI24tr645", "NKI24tr1400"))) %>%
  ungroup() %>%
...
det.title = "A. Comparing Conditional and Causal Approaches for \nBatch Effect Detection"
pval.cond_vs_adj <- pval_hmap_chart(results, dset.ord.site,
                                    categories=c("Conditional (Non-Causal)",
                                                 "Adjusted (Causal)"),
                                    category.ref="Adjusted (Causal)",
                                    title=det.title)

scanner.info <- dset.ord.site %>% 
  select(Dataset, "Scan.Manuf", "Scan.Model", Continent) %>%
...
results.scanner <- results %>%
  ...
  mutate(Same.Scanner = Scanner.Trt == Scanner.Ctrl,
         Same.Cont=Continent.Trt == Continent.Ctrl)
...
pval.hists <- results.scanner %>%
  ...
  mutate(Overlap.Group=ifelse(!Same.Cont, "Low Overlap, Different Continent", 
                              ifelse(Overlap > .05, "High Overlap, Same Continent",
                                     "Low Overlap, Same Continent"))) %>%
  group_by(Method, Scanner.Rel, Overlap.Group) %>%
  summarize(`Confounded`=sum(is.na(p.value))/n(), 
            Reject=sum(p.value < .05, na.rm=TRUE)/n(), 
            `Fail to reject`=sum(p.value > .05, na.rm=TRUE)/n()) %>%
  pivot_longer(`Confounded`:`Fail to reject`, values_to="Fraction", 
               names_to="Test Outcome") %>%
  mutate(`Test Outcome`=factor(`Test Outcome`, 
                               levels=c("Reject", "Fail to reject", "Confounded"),
                               ordered=TRUE)) %>%
  mutate(`Overlap.Group`=factor(`Overlap.Group`, 
                                levels=c("High Overlap, Same Continent", 
                                         "Low Overlap, Same Continent", 
                                         "Low Overlap, Different Continent"),
                                ordered=TRUE)) %>%
