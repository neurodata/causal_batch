...
```{r}
cor.sim <- cb.correct.caus_cComBat(sim$Ys, sim$Ts, data.frame(Covar=sim$Xs), match.form="Covar")
```

We can plot the augmented data after causal conditional ComBat has been applied, like so:

```{r, fig.width=5, fig.height=3}
plot.sim(cor.sim$Ys.corrected, cor.sim$Ts, cor.sim$Xs$Covar, title="Sigmoidal Simulation (causal cComBat corrected)")
```
